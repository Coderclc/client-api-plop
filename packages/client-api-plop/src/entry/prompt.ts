...
export const getEntryGenerator = (plop: NodePlopAPI): PlopGeneratorConfig => {
  const { Method, DOMAIN } = plop.getDefaultInclude() as Record<string, any>;
...
export const entryLambdaActionType: CustomActionFunction = (
...
export const entryLibActionType: CustomActionFunction = (
  ...args: Parameters<CustomActionFunction>
) => {
  return customAction(
    (apiName, content) => {
